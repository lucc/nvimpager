...
describe("lua functions", function()
  ...
  describe("tokenize", function()
    ...
    it("recognizes 24 bit foreground color sequences constructed with colons", function()
      test("38:2:0:162:31", {{"foreground", "0", "162", "31"}})
    end)
    it("recognizes 24 bit background color sequences constructed with colons", function()
      test("48:2:0:162:31", {{"background", "0", "162", "31"}})
    end)
  ...
  describe("ansi parser", function()
	...
	assert.equal("#141e28", state.background)
      ...
      it("parses 24 bit foreground colors with colons", function()
	state:parse("38:2:100:200:250")
	assert.equal("#64c8fa", state.foreground)
      end)
      it("parses simple 24 bit background colors with colons", function()
	state:parse("48:2:20:30:40")
	assert.equal("#141e28", state.background)
      end)
    end)
	...
	assert.equal(250, state.ctermbg)
      ...
      it("defined with colons (fg)", function()
	state:parse("38:5:10")
	assert.equal("lightgreen", state.foreground)
	assert.equal(10, state.ctermfg)
      end)
      it("defined with colons (bg)", function()
	state:parse("48:5:250")
	assert.equal("#bcbcbc", state.background)
	assert.equal(250, state.ctermbg)
      end)
